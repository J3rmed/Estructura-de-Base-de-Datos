...
class KDTree:
    ...
    def _build(self, points, depth):
        ...
        # Determine the median and split
        return Node(
    ...
    def _nearest_neighbor(self, node, target, depth):
        ...
        # Explorar el "buen" lado
        self._nearest_neighbor(good_side, target, depth + 1)
        
        # Ver si hay chances de que un punto mejor esté en el "mal" lado
        if abs(target[axis] - node.point[axis]) < self.best_dist:
            self._nearest_neighbor(bad_side, target, depth + 1)


class BruteForce:
    """Implementación ingenua basada en listas para evaluar el rendimiento esperado y verificar validaciones."""
    def __init__(self, points):
